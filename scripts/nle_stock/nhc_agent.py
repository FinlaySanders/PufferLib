...
# Reconstruction settings must match the certified ones. The stall cap in
# particular: the derive truncates an episode after this many keys without the
# game clock moving, and at its default of 1000 that fires long before the
# challenge's own 10,000-step no-progress rule, cutting games short. Under the
...
class ChallengeRunner:
    def __init__(self, weights, seed=None, max_steps=None, verbose=False, keylog=None):
    ...
    def _on_reset(self, _ud):
        ...
        try:
            ...
            if os.environ.get("NHC_ZERO_STATE"):   # bisect probe: force the policy's recurrent state to zero at every reset
...
if __name__ == "__main__":
    ...
    r = ChallengeRunner(os.path.abspath(a.weights), seed=a.seed, verbose=a.verbose, keylog=a.keylog)
