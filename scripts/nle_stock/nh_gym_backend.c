...
void nhg_keylog(const char* path) { if (g_klog) { fclose(g_klog); g_klog = NULL; } if (path && *path) g_klog = fopen(path, "w"); }
static long g_nbound, g_nafter, g_nrefresh;

void nhg_set_callback(nhg_send_cb cb, nhg_reset_cb rc, void* ud) { g_send = cb; g_resetcb = rc; g_ud = ud; g_verbose = getenv("NH_GYM_VERBOSE") != NULL; }
...
static int dr_identity_probe(GInst* g) {
    ...
    if (getenv("NH_GYM_IDLOG"))
        fprintf(stderr, "IDPROBE: got=%d role=%d race=%d gender=%d align=%d\n", got, g->S.role, g->S.race, g->S.gender, g->S.align);
    return got;
}
...
nle_ctx_t* nle_start(nle_obs* obs, FILE* f, nle_settings* set) {
    ...
    if (g_klog) fprintf(g_klog, "S role=%d race=%d gender=%d align=%d\n", g->S.role, g->S.race, g->S.gender, g->S.align);
    g_started = 1;
    return (nle_ctx_t*)g;
}
...
vmore_skip:
    ...
    if (!g->done && !stalled) {
        static int nomid = -1; if (nomid < 0) nomid = getenv("NH_GYM_NOMIDLOOK") != NULL;
        if (!nomid && dr_hero_moved(&g->S, &g->d) && !dr_prompt_open(&g->d) && !g->S.count_pending
    ...
    sync_status(g, obs);
    if (g_klog) fprintf(g_klog, "K %d %d\n", key, g->done);
    return c;
}
...
nle_ctx_t* nle_obs_refresh(nle_ctx_t* c, nle_obs* obs) {
    ...
    g->mapped = 1;
    if (g_klog) { unsigned long pp[9]; dr_obs_hash_parts(&g->d, pp); fprintf(g_klog, "R"); for (int i = 0; i < 8; i++) fprintf(g_klog, " %lx", pp[i]); fprintf(g_klog, "\n"); fflush(g_klog); }
    return c;
}
