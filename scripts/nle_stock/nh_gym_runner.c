...
// force the recurrent state to zero (demo_step_once already does this on terminal;
// this is the bisect probe for "is the leak the policy's hidden state?")
void nhg_zero_state(void) {
    if (g_net && g_net->mingru)
        memset(g_net->mingru->state, 0,
               (size_t)g_net->num_layers * g_net->hidden_size * sizeof(float));
}
