// Derivation of the fork-only NetHack observation channels from stock NLE's public
// interface (port of scripts/nle_stock/eval_stock.py). Shared by the stock vec backend
// (nh_stock_backend.c) and the fork-engine validation harness (nh_fork_harness.c).
...
#define DR_LEVELS 40
#define DR_PATH 512
// keys without game time before the episode is truncated. The fork env never truncates (its watchdog only fires ESC
// bursts), so the parity setting is a very large cap: NH_STOCK_STALLCAP=<n> (default 1000 = the python bridge rule)
static int dr_stall_cap(void) { static int cap = -1; if (cap < 0) { const char* v = getenv("NH_STOCK_STALLCAP"); cap = v ? atoi(v) : 1000; if (cap < 1) cap = 1000; } return cap; }
...
typedef struct {
    ...
    short map_raw[DR_CELLS], map_out[DR_CELLS]; int map_valid; long map_chg[DR_CELLS]; long map_gen; // export counter; a cell reprinted at this or the previous export is mapped with the newest slot knowledge (identification and redraw share a step) // lazy map mapping: the fork applies maybe_true_glyph when a cell is printed, so an identified type keeps its slot glyph on cells not reprinted since
...
static __thread int dr_log_this; // backend sets it for the instance being diagnosed
...
static int dr_resolve_item(DState* S, const char* t0, int memglyph, int* food, int* cont) { int r = dr_resolve_item_inner(S, t0, memglyph, food, cont); { static int dv = -1; if (dv < 0) dv = getenv("NH_STOCK_DISCLOG") != NULL; if (dv && dr_log_this) fprintf(stderr, "resolve \"%.50s\" -> %d (%s)\n", t0, r, r >= OBJ_OFF && r < CMAP_OFF ? NHT_OBJ_NAME[r - OBJ_OFF] : "-"); } return r; }
static int dr_resolve_item_inner(DState* S, const char* t0, int memglyph, int* food, int* cont) {
    ...
    *food = 0; *cont = 0;
    ...
    { int lc = cls; int mc = memobj ? NHT_OBJ_CLASS[memglyph - OBJ_OFF] : -1;
      ...
      if (j >= 0 && !getenv("NH_STOCK_FIRSTSLOT")) { // several types wear this description: the fork shows the true type's slot; take the most common one
...
static void dr_track_engraving(DState* S, const DObs* o, int key, const char* msg) {
    ...
    static int hitwipe = -1; if (hitwipe < 0) hitwipe = getenv("NH_STOCK_HITWIPE") != NULL; // this engine has no monster-hit wipe (u_wipe_engr is hero-caused only); the fork export flags dirty on hero wipes
...
// diagnosis knob: NH_STOCK_NOPROBE=all|look|spells|disc disables a probe family (replay divergence attribution)
static int dr_noprobe(const char* which) { static const char* e; static int init; if (!init) { init = 1; e = getenv("NH_STOCK_NOPROBE"); } return e && (!strcmp(e, "all") || !strcmp(e, which)); }
...
static int dr_look_here(DState* S, DObs* o, void* ctx, dr_send_fn send, DLook* res) {
    ...
    if (dr_noprobe("look")) { S->last_skip = 2; return 0; }
    ...
    // unless NH_STOCK_NOBLINDLOOK=1
    // REFUTED 2026-09-09: NH_STOCK_BLINDLOOK=1 allows the blind look; seed 7 then RNG-diverges in 49/84 episodes (exact 25 -> 14)
    static int blindok = -1; if (blindok < 0) blindok = getenv("NH_STOCK_BLINDLOOK") != NULL;
    if ((cond & 0x200) || (!blindok && (cond & 0x20))) { S->probe_skip_blind++; S->last_skip = 3; return 0; }
    ...
    for (int it = 0; it < 10; it++) {
        { static long dump = -2; if (dump == -2) { const char* v = getenv("NH_STOCK_LOOKDUMP"); dump = v ? atol(v) : -1; } if (dump >= 0 && dr_log_this && (o->blstats[20] == dump || dump == 0)) { fprintf(stderr, "lookdump T=%ld it=%d misc=%d%d%d cursor=%d,%d msg=\"%.70s\"\n", (long)o->blstats[20], it, o->misc[0], o->misc[1], o->misc[2], o->tty_cursor ? (int)o->tty_cursor[0] : -1, o->tty_cursor ? (int)o->tty_cursor[1] : -1, (const char*)o->message); for (int r = 0; r < DR_TTY_LI; r++) { char row[DR_TTY_CO + 1]; dr_row(o, r, row); int nb = 0; for (int c = 0; row[c]; c++) if (row[c] != ' ') nb++; if (nb) fprintf(stderr, "  row%02d: %s\n", r, row); } } }
...
#define DR_FLUSH_SEG() do { while (sl && seg[sl - 1] == ' ') sl--; seg[sl] = 0; int b = 0; while (seg[b] == ' ') b++; if (sl > b) { const char* t = seg + b; \
        ...
        DR_FLUSH_SEG();
#undef DR_FLUSH_SEG
        { static int rl = -1; if (rl < 0) rl = getenv("NH_STOCK_RAWLOG") != NULL; if (rl && dr_log_this) fprintf(stderr, "rawlook T=%ld bytes=%d items=%d tty_items=%d first=\"%s\"\n", o->blstats[20], n, rn, nw, rn ? rw[0] : ""); }
...
static void dr_probe_peaceful(DState* S, DObs* o, void* ctx, dr_send_fn send) {
    ...
    { static int pv = -1; if (pv < 0) pv = getenv("NH_STOCK_PEACELOG") != NULL; if (pv) { int nm = 0; for (int k = 0; k < DR_CELLS; k++) if (o->glyphs[k] >= 0 && o->glyphs[k] < PET_OFF && k != hk) nm++; fprintf(stderr, "peace scan T=%ld need=%d monsters=%d cursor=%d\n", T, need, nm, o->tty_cursor != NULL); } }
    if (!need) { S->peace_dirty = 0; return; }
    if (S->count_pending || dr_noprobe("peace") || dr_prompt_open(o) || (o->blstats[25] & 0x220) || !o->tty_cursor) { S->probes_deferred++; { static int pv = -1; if (pv < 0) pv = getenv("NH_STOCK_PEACELOG") != NULL; if (pv) fprintf(stderr, "peace T=%ld deferred need=%d count=%d prompt=%d blind=%ld cursor=%d\n", T, need, S->count_pending, dr_prompt_open(o), o->blstats[25] & 0x220, o->tty_cursor != NULL); } return; }
    DSave sv; dr_save(o, &sv);
...
// '\' discoveries
static void dr_note_discovered(DState* S, int idx) { if (idx >= 0 && idx < NUM_OBJECTS && (g_shuffled[idx] || NHT_OBJ_DESCR[idx][0])) { { static int dv = -1; if (dv < 0) dv = getenv("NH_STOCK_DISCLOG") != NULL; if (dv && dr_log_this && NHT_OBJ_CLASS[idx] == 13) fprintf(stderr, "gemdisc idx=%d %s was=%d\n", idx, NHT_OBJ_NAME[idx], S->discovered[idx]); } S->discovered[idx] = 1; } }
static void dr_probe_discoveries(DState* S, DObs* o, void* ctx, dr_send_fn send) {
    ...
    rows_seen = 0;
    { static int dv3 = -1; if (dv3 < 0) dv3 = getenv("NH_STOCK_DISCRAW") != NULL;
      if (dv3 && dr_log_this) { char dbgm[256]; dr_msg(o, dbgm, sizeof dbgm);
        fprintf(stderr, "discopen T=%ld try=%d misc=%d%d%d cond=%lx msg=\"%s\"\n", o->blstats[20], attempt,
                o->misc ? o->misc[0] : -1, o->misc ? o->misc[1] : -1, o->misc ? o->misc[2] : -1,
                (unsigned long)o->blstats[25], dbgm); } }
    send(ctx, '\\'); S->probe_keys += 2;
    ...
    for (int it = 0; it < 96; it++) {
        for (int r = 0; r < DR_TTY_LI; r++) {
            ...
            { static int dv4 = -1; if (dv4 < 0) { const char* v = getenv("NH_STOCK_DISCTTY"); dv4 = v ? atoi(v) : -1; }
              if (dv4 > 0 && dr_log_this && o->blstats[20] == dv4) fprintf(stderr, "disctty T=%ld r=%02d [%s]\n", o->blstats[20], r, row); }
            ...
            { static int dv2 = -1; if (dv2 < 0) dv2 = getenv("NH_STOCK_DISCRAW") != NULL;
              if (dv2 && dr_log_this && *s) fprintf(stderr, "discraw T=%ld %s |%s|\n", o->blstats[20],
                  (!par ? "NOPAREN " : (s[L-1] != ')' ? "NOCLOSE " : "ok      ")), s); }
            if (!par) continue; if (s[L - 1] != ')') continue;
            { static int dv = -1; if (dv < 0) dv = getenv("NH_STOCK_DISCLOG") != NULL; if (dv && dr_log_this) fprintf(stderr, "discrow T=%ld \"%s\"\n", o->blstats[20], s); }
            rows_seen++;
            ...
            *par = 0; if (strstr(s, " called ")) continue; int idx = dr_name_index(s); dr_note_discovered(S, idx);
            if (idx >= 0) { char dl[64]; dr_lower(dl, descr, sizeof dl); int slot = dr_appearance_descr_c(dl, NHT_OBJ_CLASS[idx]); if (slot >= 0) dr_learn_type_src(S, slot, idx, 2); { static int dv = -1; if (dv < 0) dv = getenv("NH_STOCK_DISCLOG") != NULL; if (dv && dr_log_this) fprintf(stderr, "disc T=%ld \"%s\" (%s) -> type %d slot %d\n", o->blstats[20], s, descr, idx, slot); } }
        }
...
static void dr_learn_type_src(DState* S, int slot, int type, int src) {
    ...
    { static int dv = -1; if (dv < 0) dv = getenv("NH_STOCK_DISCLOG") != NULL; if (dv && dr_log_this && S->type_of_descr[slot] != type) fprintf(stderr, "learn slot %d (%s) -> type %d (%s)\n", slot, NHT_OBJ_DESCR[slot], type, NHT_OBJ_NAME[type]); }
...
static int dr_learn_named(DState* S, int g, const char* text, int oc) {
    ...
    { static int dv5 = -1; if (dv5 < 0) dv5 = getenv("NH_STOCK_NAMEDLOG") != NULL;
      if (dv5 && dr_log_this) fprintf(stderr, "named g=%d slot=%d oc=%d text=\"%.60s\"\n", g, g - OBJ_OFF, oc, text); }
...
static int dr_after_key(DState* S, DObs* o, void* ctx, dr_send_fn send, int key, int done) {
    ...
    if (!done) {
        ...
        { static long cw = -2; if (cw == -2) { const char* v = getenv("NH_STOCK_CELLWATCH"); cw = v ? atol(v) : -1; } if (cw >= 0 && cw < DR_CELLS && dr_log_this) { DLevel* L = dr_level(S, o); fprintf(stderr, "cellwatch T=%ld key=%d hero=%d,%d cell%ld raw=%d terr=%d litmark=%d\n", o->blstats[20], key, (int)o->blstats[0], (int)o->blstats[1], cw, o->glyphs[cw], L->terr[cw], L->litmark[cw]); } }
    }
...
static int dr_door_side(const DState* S, const DLevel* L, int row, int col) {
    if (!S->d_valid || getenv("NH_STOCK_NODOORSIDE")) return -1;
...
static int dr_guess_terrain(const DState* S, const DLevel* L, int row, int col) {
    ...
    { static int nocorner = -1; if (nocorner < 0) nocorner = getenv("NH_STOCK_NOCORNER") != NULL;
      static const int dd[4][2] = {{-1, -1}, {-1, 1}, {1, -1}, {1, 1}}; static const int want[4] = {3, 4, 5, 6};
      if (!nocorner && !corr_nb) for (int i = 0; i < 4; i++) { int r = row + dd[i][0], c = col + dd[i][1]; if (r < 0 || r >= DR_ROWS || c < 0 || c >= DR_COLS) continue; if (L->terr[r * DR_COLS + c] == CMAP_OFF + want[i]) return 19; } }
    if (corr_nb && !room_nb) { for (int i = 0; i < 4; i++) { int r = row + dr[i], c = col + dc[i]; if (r < 0 || r >= DR_ROWS || c < 0 || c >= DR_COLS) continue; if (L->terr[r * DR_COLS + c] == CMAP_OFF + 22) return 22; }
        static int nolit2 = -1; if (nolit2 < 0) nolit2 = getenv("NH_STOCK_NOLIT2") != NULL;
        if (!nolit2) { int lit = 0, unlit = 0; for (int r = row - 2; r <= row + 2; r++) for (int c = col - 2; c <= col + 2; c++) { if (r < 0 || r >= DR_ROWS || c < 0 || c >= DR_COLS || (r == row && c == col)) continue; int v = L->terr[r * DR_COLS + c]; if (v == CMAP_OFF + 22) lit++; else if (v == CMAP_OFF + 21) unlit++; } if (lit > unlit) return 22; }
        return 21; } // S_litcorr marks a permanently lit square (waslit = lev->lit, e.g. a light-scroll pool): pools are contiguous
    if ((prev == 21 || prev == 22 || (prev >= 12 && prev <= 16)) && (!room_nb || getenv("NH_STOCK_CORRGUESS"))) return 21; // refuted: 'undisplayed + came from a door = corridor' broke 89 episodes before turn 200 (dark rooms); keep the room-neighbour veto
...
static int dr_probe_terrain(DState* S, DObs* o, void* ctx, dr_send_fn send, int k) {
    ...
    { static int on = -1; if (on < 0) { const char* e = getenv("NH_STOCK_TERRAIN_PROBE"); on = e && *e && *e != (char)48; } if (!on) return -1; }
    if (dr_noprobe("terrain") || dr_prompt_open(o)) return -1;
    ...
    { static int nh = -1; if (nh < 0) nh = getenv("NH_STOCK_NOTERRHARVEST") != NULL;
      if (!nh) { DLevel* HL = dr_level(S, o);
    ...
    { static int pl = -1; if (pl < 0) pl = getenv("NH_STOCK_PILELOG") != NULL; if (pl && dr_log_this) { char m[256]; dr_msg(o, m, sizeof m); fprintf(stderr, "terrprobe T=%ld cell=%d view=%d t=%d misc=%d%d%d msg=\"%.60s\"\n", o->blstats[20], k, v, t, o->misc[0], o->misc[1], o->misc[2], m); } }
...
static void dr_mark_lit(DState* S, const DObs* o) {
    ...
    { static int pl = -1; if (pl < 0) pl = getenv("NH_STOCK_PILELOG") != NULL; if (pl && dr_log_this) { int n = 0; for (int i = 0; i < DR_CELLS; i++) n += L->litmark[i]; static long cw = -2; if (cw == -2) { const char* v = getenv("NH_STOCK_CELLWATCH"); cw = v ? atol(v) : -1; }
        fprintf(stderr, "litmarks T=%ld hero=%d,%d total=%d", o->blstats[20], col, row, n); if (cw >= 0) { int r = (int)(cw / DR_COLS), c = (int)(cw % DR_COLS); fprintf(stderr, " watch cell%ld r=%d c=%d d2=%d clear=%d raw=%d terr=%d", cw, r, c, (r - row) * (r - row) + (c - col) * (c - col), dr_clear_path(L, o, row, col, r, c), o->glyphs[cw], L->terr[cw]); } fprintf(stderr, "\n"); } }
}
...
static void dr_boundary(DState* S, DObs* o, void* ctx, dr_send_fn send) {
    ...
    // NH_STOCK_NOEXTRALOOKS=1 drops the periodic re-look (it marks floor objects dknown): REFUTED, seed 7 exact 25 -> 12
    static int noextra = -1; if (noextra < 0) noextra = getenv("NH_STOCK_NOEXTRALOOKS") != NULL;
    ...
    int acted = moved || S->cell_dirty || sight_back || hero_redrawn || dr_cell_event(m0) || o->blstats[20] <= 1 || (!noextra && o->blstats[20] - S->last_look_turn >= 10) || on_corpse; // periodic revalidation: objects under the hero can vanish silently (corpses rot)
    ...
    { static int pl = -1; if (pl < 0) pl = getenv("NH_STOCK_PILELOG") != NULL; if (pl && dr_log_this) { fprintf(stderr, "cell T=%ld cond=%lx moved=%d dirty=%d acted=%d have=%d skip=%d nobj=%d objm=%d raw=%d Lterr=%d res_terr=%d", o->blstats[20], (unsigned long)o->blstats[25], moved, S->cell_dirty, acted, have, S->last_skip, have ? res.nobj : -1, inb ? L->objm[k] : -2, inb ? o->glyphs[k] : -2, inb ? L->terr[k] : -2, have ? res.terrain : -3); if (have) for (int i = 0; i < res.nobj && i < 4; i++) fprintf(stderr, " [%d]=\"%.40s\"", i, res.objs[i]); if (have) { fprintf(stderr, " engr=%d", res.engr); for (int i = 0; i < res.nraw; i++) fprintf(stderr, " L%d=\"%.60s\"", i, res.raw[i]); } fprintf(stderr, " msg=\"%.50s\"\n", m0); } }
    ...
    if (have) {
        ...
        if (t < 0) {
            ...
            // beats probing and guessing; NH_STOCK_NOTERRMEM=1 restores the corridor-only rule
            static int noterrmem = -1; if (noterrmem < 0) noterrmem = getenv("NH_STOCK_NOTERRMEM") != NULL;
            ...
            static int ncp = -1; if (ncp < 0) ncp = getenv("NH_STOCK_NOCORRPROBE") != NULL; // the lit-corridor probe may read waslit, not lit
            if (!ncp && mt == 21 && !L->corr_seen[k] && (o->blstats[25] & 0x3a0) == 0) {
            ...
            else if (!noterrmem && !guessed && mt >= 12 && mt <= 41 && mt != 19 && mt != 20) t = mt;
            else if ((t = dr_probe_terrain(S, o, ctx, send, k)) >= 0) { L->terr[k] = (short)(CMAP_OFF + t); L->guess[k] = 0; }
            else if (guessed && (mt == 21 || mt == 22 || (!noterrmem && mt >= 12 && mt <= 41 && mt != 19 && mt != 20))) t = mt;
        ...
        if ((ate && S->top >= 0 && ((S->top >= BODY_OFF && S->top < BODY_OFF + NUMMONS) || (S->top >= OBJ_OFF && S->top < CMAP_OFF && dr_is_food(S->top)))) || took) {
            ...
            static int noqty = -1; if (noqty < 0) noqty = getenv("NH_STOCK_NOPILEQTY") != NULL;
            if (ate && !took && !noqty && S->npile > 0 && S->pile_qty[0] > 1) { S->pile_qty[0]--; /* one of a stack eaten: the rest stays on top */ }
        ...
        // NH_STOCK_KEEPDARK=1: stop folding dark floor into lit floor. back_to_glyph reports S_darkroom for an unlit room
        // square, and we were reporting lit floor for it -- 6 of the 17 wrong-terrain divergences.
        { static int kd = -1; if (kd < 0) kd = getenv("NH_STOCK_KEEPDARK") != NULL;
          if (!kd && t == CMAP_OFF + 20) t = CMAP_OFF + 19; }
    ...
    { static int shown; if (getenv("NH_DERIVE_INVLOG") && shown < 12 && n) { shown++; fprintf(stderr, "INVTEXT n=%d [0]=\"%s\" oc=%d g=%d\n", n, items[0].t, items[0].oc, items[0].g); } }
...
static inline short dr_map_glyph(const DState* S, short g) {
    if (g >= OBJ_OFF && g < CMAP_OFF) { int i = g - OBJ_OFF; if (!S->discovered[i] && dr_gem_canon(i) != i) { { static int dv = -1; if (dv < 0) dv = getenv("NH_STOCK_DISCLOG") != NULL; if (dv && dr_log_this && dr_gem_canon(i) != i) fprintf(stderr, "gemcanon %d (%s) -> %d\n", i, NHT_OBJ_NAME[i], dr_gem_canon(i)); } return (short)(OBJ_OFF + dr_gem_canon(i)); } int t = S->type_of_descr[i]; if (t >= 0) return (short)(OBJ_OFF + t); }
    return g;
}
static inline short dr_map_glyph_prev(const DState* S, short g) {
    if (g >= OBJ_OFF && g < CMAP_OFF) { int i = g - OBJ_OFF; if (!S->discovered[i] && dr_gem_canon(i) != i) return (short)(OBJ_OFF + dr_gem_canon(i)); int t = S->type_prev[i]; if (t >= 0) return (short)(OBJ_OFF + t); }
    return g;
}
static void dr_export_glyphs(DState* S, DObs* o, int with_hero_tile) {
    static int lazymap = -1; if (lazymap < 0) lazymap = getenv("NH_STOCK_LAZYMAP") != NULL; // the fork now maps identities eagerly at every fill; NH_STOCK_LAZYMAP=1 restores the reprint model
    if (!S->map_valid || !lazymap) { for (int k = 0; k < DR_CELLS; k++) { S->map_raw[k] = o->glyphs[k]; S->map_out[k] = o->glyphs[k] = dr_map_glyph(S, o->glyphs[k]); } S->map_valid = 1; }
    else { S->map_gen++; for (int k = 0; k < DR_CELLS; k++) { short raw = o->glyphs[k]; if (raw != S->map_raw[k]) { S->map_raw[k] = raw; S->map_chg[k] = S->map_gen; }
        // the fork applies identities when a cell is printed: with the print_glyph hook a cell is remapped when it was reprinted
        // (a redraw that keeps the same appearance glyph is invisible in the raw obs); without it, on raw change plus one export
        if (o->redrawn ? (o->redrawn[k] || S->map_chg[k] == S->map_gen) : (S->map_chg[k] >= S->map_gen - 1)) S->map_out[k] = (S->map_chg[k] == S->map_gen && !getenv("NH_STOCK_NOPREVMAP")) ? dr_map_glyph_prev(S, raw) : dr_map_glyph(S, raw); o->glyphs[k] = S->map_out[k]; }
        { static int tr = -1; if (tr < 0) tr = getenv("NH_STOCK_TRACE") != NULL; if (tr && dr_log_this && o->redrawn) { int n = 0; char b[400]; int p = 0; for (int k = 0; k < DR_CELLS; k++) if (o->redrawn[k]) { n++; if (p < 380) p += snprintf(b + p, sizeof b - (size_t)p, "%d,", k); } fprintf(stderr, "redrawn T=%ld n=%d cells=%s\n", o->blstats[20], n, b); } }
        if (o->redrawn) memset(o->redrawn, 0, DR_CELLS); }
    ...
    if (with_hero_tile && S->d_valid) {
        ...
        static int blindpile = -1; if (blindpile < 0) blindpile = getenv("NH_STOCK_BLINDPILE") != NULL;
        ...
        else if (blindpile || !(o->blstats[25] & 0x220)) o->glyphs[S->d_r * DR_COLS + S->d_c] = dr_map_glyph(S, (short)under); // 0x220 = blind|hallucinating: the fork now conceals the hero's own cell for both (a hallucinating hero can no more identify what is underfoot than a blind one), so leave stock's raw hero glyph
...
// ---------------------------------------------------------------- replay hash (engine-invariant view)
static unsigned long dr_obs_hash(const DObs* o) {
    unsigned long h = 1469598103934665603UL;
#define H8(b) (h = (h ^ (unsigned long)(unsigned char)(b)) * 1099511628211UL)
    for (int i = 0; i < 27; i++) { long v = o->blstats[i]; for (int k = 0; k < 8; k++) H8(v >> (8 * k)); }
    int hr = (int)o->blstats[1], hc = (int)o->blstats[0];
    int cr = o->tty_cursor ? (int)o->tty_cursor[0] - 1 : -1, cc = o->tty_cursor ? (int)o->tty_cursor[1] : -1; // the cursor sits on the hero
    for (int k = 0; k < DR_CELLS; k++) { int g = o->glyphs[k]; int r = k / DR_COLS, c = k % DR_COLS; if ((r == hr && c == hc) || (r == cr && c == cc)) continue; if (g >= OBJ_OFF && g < CMAP_OFF) g = OBJ_OFF; if (g >= BODY_OFF && g < BODY_OFF + NUMMONS) g = BODY_OFF; if (g >= STATUE_OFF && g < STATUE_OFF + NUMMONS) g = STATUE_OFF; H8(g); H8(g >> 8); }
    for (int i = 0; i < DR_INV; i++) { H8(o->inv_letters[i]); H8(o->inv_oclasses[i]); if (!o->inv_letters[i]) break; }
#undef H8
    return h;
}
// Level-2 hash: everything the policy observation is built from, nothing masked (message, every map cell including
// the hero's, inventory glyphs/letters/classes, the fork's 11 internal slots, per-slot state, true identities, misc,
// cursor). Selected with NH_HASH_FULL=1 on both the fork logger and the stock replayer.
static unsigned long dr_obs_hash_full(const DObs* o) {
    unsigned long h = 1469598103934665603UL;
#define H8(b) (h = (h ^ (unsigned long)(unsigned char)(b)) * 1099511628211UL)
#define HL(v) do { long _v = (long)(v); for (int _k = 0; _k < 8; _k++) H8(_v >> (8 * _k)); } while (0)
#define H16(v) do { int _v = (int)(v); H8(_v); H8(_v >> 8); } while (0)
    for (int i = 0; i < 27; i++) HL(o->blstats[i]);
    for (int k = 0; k < DR_CELLS; k++) H16(o->glyphs[k]);
    for (int i = 0; i < 256; i++) { H8(o->message[i]); if (!o->message[i]) break; }
    for (int i = 0; i < DR_INV; i++) { H16(o->inv_glyphs[i]); H8(o->inv_letters[i]); H8(o->inv_oclasses[i]); }
    if (o->internal) { HL(o->internal[6]); HL(o->internal[7]); } // the env reads only these two slots
    if (o->inv_state) for (int i = 0; i < DR_INV * 8; i++) H8(o->inv_state[i]);
    if (o->inv_true) for (int i = 0; i < DR_INV; i++) H16(o->inv_true[i]);
    if (o->misc) for (int i = 0; i < 3; i++) HL(o->misc[i]);
#undef H16
#undef HL
#undef H8
    return h;
}
// per-field-group hashes for attributing a full-hash difference (same groups as dr_obs_hash_full)
static const char* dr_part_names[9] = {"blstats", "glyphs", "message", "inv", "internal", "inv_state", "inv_true", "misc", "cursor"};
static void dr_obs_hash_parts(const DObs* o, unsigned long out[9]) {
    for (int g = 0; g < 9; g++) out[g] = 1469598103934665603UL;
#define H8g(g, b) (out[g] = (out[g] ^ (unsigned long)(unsigned char)(b)) * 1099511628211UL)
#define HLg(g, v) do { long _v = (long)(v); for (int _k = 0; _k < 8; _k++) H8g(g, _v >> (8 * _k)); } while (0)
#define H16g(g, v) do { int _v = (int)(v); H8g(g, _v); H8g(g, _v >> 8); } while (0)
    for (int i = 0; i < 27; i++) HLg(0, o->blstats[i]);
    for (int k = 0; k < DR_CELLS; k++) H16g(1, o->glyphs[k]);
    for (int i = 0; i < 256; i++) { H8g(2, o->message[i]); if (!o->message[i]) break; }
    for (int i = 0; i < DR_INV; i++) { H16g(3, o->inv_glyphs[i]); H8g(3, o->inv_letters[i]); H8g(3, o->inv_oclasses[i]); }
    if (o->internal) { HLg(4, o->internal[6]); HLg(4, o->internal[7]); }
    if (o->inv_state) for (int i = 0; i < DR_INV * 8; i++) H8g(5, o->inv_state[i]);
    if (o->inv_true) for (int i = 0; i < DR_INV; i++) H16g(6, o->inv_true[i]);
    if (o->misc) for (int i = 0; i < 3; i++) HLg(7, o->misc[i]);
#undef H16g
#undef HLg
#undef H8g
}
static unsigned long dr_hash_sel(const DObs* o) {
    static int mode = -1;
    if (mode < 0) { const char* e = getenv("NH_HASH_FULL"); mode = (e && *e && *e != '0') ? 1 : 0; }
    return mode ? dr_obs_hash_full(o) : dr_obs_hash(o);
}
#endif
