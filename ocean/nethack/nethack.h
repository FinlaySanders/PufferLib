...
static int nethack_slot_usable(const Nethack* env, const Verb* verb, int i) {
    if (env->cant_hold && (verb == &NETHACK_VERBS[NETHACK_ACT_WIELD] || verb == &NETHACK_VERBS[NETHACK_ACT_THROW]
                           || (verb == &NETHACK_VERBS[NETHACK_ACT_WEAR] && !nethack_no_refusal_masks()))) return 0;
...
static void nethack_compute_mask(Nethack* env) {
    ...
    // zero-time memory (nethack_zt_*): item verbs were filtered per slot in nethack_slot_usable above; here the
    // directional verbs lose the refused direction and verbs without heads switch off, until the world changes
    if (nethack_zt_mask_on() && !nethack_no_refusal_masks() && env->zt_n && nethack_zt_world_same(env)) {
        for (int k = 0; k < env->zt_n; k++) {
            ...
            if (dh >= 0 && env->zt_dir[k] >= 0 && same_verb < 3) {
                ...
                if (!left) { mask[v] = 0; row[0] = 1; } // verb off; the unconsumed head keeps a dummy legal entry
            } else mask[v] = 0;
        }
        int any = 0;
...
void puf_step(Nethack* env) {
    ...
    nethack_track_cant_hold(env); // the refusal text is the last key's message; prompt handling below may blank it
    nethack_handle_prompts(env);
    if (!env->obs.done) nle_obs_refresh(env->ctx, &env->obs);
    if (env->blstats[NLE_BL_TIME] == time_before) { if (++env->noprog_run > env->noprog_max) env->noprog_max = env->noprog_run; }
    else env->noprog_run = 0;
    // zero-time memory: an action that spent no clock and moved nothing was refused, whatever the engine said (running
    // into a wall stops silently). Remember the exact action while the world stays as it is; forget when it changes.
